...
void RA8875::begin(const enum RA8875sizes s,uint8_t colors) 
{
	...
	_currentMode = false; //Start in text mode
	_brightness = 255;
...
/*	Font Control Register 1 [0x22]
	...
	*/
	...
	//------------------------------- Start SPI initialization ------------------------------------------
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK66FX1M0__)//Teensy 3,3.1
		...
		#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MKL26Z64__) || defined(__MK66FX1M0__)//all Tensy's 32bit
			pinMode(_cs, OUTPUT);
...
void RA8875::initialize() 
{
	...
	setFont(INTERN);	//set internal font use
	//postburner PLL!
	...
	setCursor(0,0);
	prompt_size = 8 * (1+_textHScale); // prompt ">"
	vdata = 0;
	numlin = _height / 16+(16 * _textVScale);
	rtmarg =  _width / 8+(8 * _textHScale);
	tab_size = 8;
	yStart = 0;
	yDraw = 0;
	dTemp = 0;
...
// The next line below fails with large filled circles. Times out to soon.
...
*/
/**************************************************************************/
void RA8875::setTextXY(int16_t x, int16_t y,bool update)
{
	setTextPosition(x*getFontWidth(), y*getFontHeight(), update);
}
...
void RA8875::setFontScale(uint8_t scale)
{
	...
	_textHScale = scale;
	_textVScale = scale;
	numlin = _height / 16 / (_textVScale + 1);
	rtmarg =  _width / 8 / (_textHScale + 1);
}
...
//[40h][41h][42h][43h]=========================================================================
void RA8875::Graphic_Cursor_XY(unsigned short WX,unsigned short HY)
{
/*
...
*/	
	writeReg(RA8875_GCHP0,WX);
	writeReg(RA8875_GCHP1,WX>>8);

	writeReg(RA8875_GCVP0,HY);
	writeReg(RA8875_GCVP1,HY>>8);
}
...
// Display a single character on the TFT
void RA8875::outChar(uint8_t text) {
...
//		_cursorY += (_FNTheight + (_FNTheight * (_scaleY-1))) + _FNTinterline;// + offset;
		_cursorY += currentFontH+(currentFontH * _textVScale);
        _cursorX = 0x00;
        update_xy();
    } else if(text == 0x08) { // Destructive Backspace
			if(_cursorX > prompt_size)
			{
				_cursorX-= currentFontW+(currentFontW * _textHScale);
				update_xy(); 
				vdata = 0x20;
				update_tft();
				update_xy();
			...
			_cursorX += (tab_size*(currentFontW * (1 + _textHScale)));
			update_xy();
...
// update display data
void RA8875::update_tft(void)
{
	const uint8_t currentFontH = 16;
	if((_cursorY ==  (_height - (currentFontH+(currentFontH * _textVScale)))) && (yStart > 0))
		dTemp = yDraw;
	else
		dTemp = _cursorY;
    setTextPosition(_cursorX,dTemp,false);
	writeCommand(RA8875_MRWC);
	writeData((char)vdata);
	waitBusy(0x80);
}

// update display coordinates and scroll screen if needed
void RA8875::update_xy(void)
{
	const uint8_t currentFontH = 16;
   if(_cursorY == _height)
   {
	  setScrollWindow(0, _width-1, 0, _height-1);
      yDraw = scroll_line();
      _cursorY -= currentFontH+(currentFontH * _textVScale);
//      _cursorY -= (_FNTheight * (_scaleY -1));
	  _cursorX = 0;
   }
   setTextPosition(_cursorX,_cursorY,true);
}

int RA8875::scroll_line(void)
{
	const uint8_t currentFontW = 8;
	const uint8_t currentFontH = 16;
	int temp = _cursorY;
	int yTemp = yStart; // Store the old yStart, this is where we draw the next line
    setTextPosition(_cursorX,yStart,false);
	if(!_currentMode){
		for(uint16_t i=0; i < (rtmarg / _textHScale); i++)
		{
			writeCommand(RA8875_MRWC);
			writeData(' ');
			waitBusy(0x80);
		...
		fillRect(0, yStart, _width,currentFontH+(currentFontH * _textVScale), _textBackColor);
	}
    setTextPosition(_cursorX,temp,false);

	// Change the top of the scroll area
	yStart += currentFontH+(currentFontH * _textVScale);
	// The value must wrap around as the screen memory is a circular buffer
	if (yStart >= _height)
		yStart = yStart - _height;
